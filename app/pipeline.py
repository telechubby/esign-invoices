"""Orchestrates the full batch: match invoices to customers, sign each one,
then email it - reporting per-file status so the GUI can show live progress.
"""
...
def build_jobs(
    ...
    for m in matches:
        ...
        elif run_log.is_sent(m.file_path.name):
            jobs.append(InvoiceJob(match=m, status=Status.ALREADY_SENT))
        else:
...
def run_batch(
    ...
    for job in jobs:
        ...
        try:
            send_invoice_email(mail_creds, customer.email, subject, body, out_path)
